...
class EmailsDocumentos extends Controlador {
    ...
    private function enviarEmailDocumentoPdf($post){
        ...
        $nombreRemitente = 'PINSOS DE VENTRE';
        $nombreEmpresa = NOMBRE_CORREO;
        $emailRemitente = CORREO_ADMIN;
        $passwordRemitente = PASSWORD_CORREO;            
        $movil = '';        
        $idDocumento = $post['idAlbaranClienteEmail'];

        
        if($emailRemitente != '' && $passwordRemitente != ''){
            ...
            $envio = enviarEmail::enviarEmailConDocumentos($nombreRemitente, $emailRemitente, $passwordRemitente, $emailsDestino, $asunto, $message, $attachmentArray);
    ...
    private function generarPdfAlbaranParaEmail($idAlbarab)
    {                       
