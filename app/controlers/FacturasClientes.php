...
//require_once 'librerias/EmailsDocumentos.php';
...
class FacturasClientes extends Controlador {
                    ...
                    if($ins){
                        ...
                        if($insRows){
                            ...
                            if(API_GOOGLE_DRIVE==1){
                                //crear pdf factura en drive
                                $factura = $this->modeloFacturaCliente->getInvoiceData($ins);
                                $numFactura = $factura->numero;
                                $nombreArchivo = "Factura_{$numFactura}.pdf";
                                GoogleDriveUploader::subirArchivo($ins, $_POST['fecha_factura_cliente'], 'Facturas Cliente', $nombreArchivo);
                            }
                        }                
                    }
                }else{
    ...
    public function actualizarFactura()
    {
        ...
        if(isset($_POST['id']) && $_POST['id'] != '' && $_POST['id'] > 0 && isset($_POST['idcliente']) && $_POST['idcliente'] > 0 && trim($_POST['nif_factura']) != '' && $_POST['fecha'] != '' && trim($_POST['dias']) != '' && $_POST['vencimiento'] != '' && isset($_POST['numeroOrden']) && count($_POST['numeroOrden']) > 0 ){
                ...
                if($updRows){                    
                    ...
                    $factAntes = new ModeloFacturaCliente();
                    $dataFactAntes = $factAntes->getInvoiceData($idFactura);
                    ...
                    if($ok && $okw){
                        ...
                        if($upd){                            
                            ...
                            if(API_GOOGLE_DRIVE==1){
                                $numFactura = $new->numero;
                                $nombreArchivo = "Factura_{$numFactura}.pdf";                            
                                GoogleDriveUploader::reemplazarArchivo($idFactura, $_POST['fecha'],  'Facturas Cliente', $nombreArchivo, $dataFactAntes->fecha);
                            }
    ...
    {        
        ...
        if(isset($_POST['idFacturaEliminar']) && $_POST['idFacturaEliminar'] > 0) {               
            ...
            $modeloFactura = $this->modeloFacturaCliente->getInvoiceData($idFactura);
            ...
            if ($updCabecera && $updFilas) {
                ...
                if(API_GOOGLE_DRIVE==1){
                    //eliminar pdf Drive
                    $numFactura = $modeloFactura->numero;
                    $nombreArchivo = "Factura_{$numFactura}.pdf";                
                    GoogleDriveUploader::eliminarArchivo($modeloFactura->fecha, 'Facturas Cliente', $nombreArchivo);
                }
