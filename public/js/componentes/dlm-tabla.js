class tabla extends HTMLElement {
    ...
    configurarPasarFechas(){
        
        let fechasString = "";
        let fechaIni = document.getElementById("fechaIni");
        let fechaFin = document.getElementById("fechaFin");      
        
        if(!fechaIni || fechaIni.value=='' || !fechaFin || fechaFin.value==''){
            fechasString = '';
        }else{
            fechasString = `BETWEEN '${fechaIni.value}' AND '${fechaFin.value}'  `;
        }
    ...
    cabecera() {
        ...
        for (let i = 0; i < this.titulos.length; i++) {
            ...
            if(JSON.parse(columnasOcultas).indexOf(i) === -1){
                visible = ' ';
            }
    ...
    montarFilasTabla(resultado) {
        ...
        for (let i = 0; i < resultado.length; i++) {
            ...
            if (acciones != '' && acciones != 'c') {
                ...
                if (acciones.includes('p')) {
                    colAcciones += `<a href="#" data-crud="p" data-idpay="${resultado[i].id}" id="${this.id}_pagarbtn${i}" class="${this.id}_pagarbtn button_pagar_tabla text-red-600 px-2 text-lg hover:text-red-300" style="cursor:pointer;">${iconosCrud['f']}</a>`;
                }
            ...
            for (const key in datos) {
                ...
                let visible = ' style="display:none" ';
                if(JSON.parse(columnasOcultas).indexOf(col) === -1){
                    visible = ' ';
                }
